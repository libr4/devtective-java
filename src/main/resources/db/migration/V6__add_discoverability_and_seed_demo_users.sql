INSERT INTO public.user_discoverability (id, code)
VALUES (2, 'WORKSPACE')
...
INSERT INTO public.worker (user_id, first_name, last_name, position_id)
SELECT u.user_id, 'Alice', 'Almeida', 4
FROM public.app_user u
WHERE u.username = 'demo.alice'
  AND NOT EXISTS (SELECT 1 FROM public.worker w WHERE w.user_id = u.user_id);

INSERT INTO public.worker (user_id, first_name, last_name, position_id)
SELECT u.user_id, 'Bob', 'Barbosa', 3
FROM public.app_user u
WHERE u.username = 'demo.bob'
  AND NOT EXISTS (SELECT 1 FROM public.worker w WHERE w.user_id = u.user_id);

INSERT INTO public.worker (user_id, first_name, last_name, position_id)
SELECT u.user_id, 'Carol', 'Costa', 2
FROM public.app_user u
WHERE u.username = 'demo.carol'
  AND NOT EXISTS (SELECT 1 FROM public.worker w WHERE w.user_id = u.user_id);

INSERT INTO public.worker (user_id, first_name, last_name, position_id)
SELECT u.user_id, 'Dave', 'Dias', 1
FROM public.app_user u
